...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Morpheus: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Morpheus Signed Message:\n";
...
{
    // Morpheus: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Morpheus: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "Morpheus Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb9, 0xde }; // Morpheus: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MorpheusMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MorpheusMiner : ProcessBlock, block not accepted");
    }
