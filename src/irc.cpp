...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MorpheusTEST0\r");
            Send(hSocket, "WHO #MorpheusTEST0\r");
        } else {
            // randomly join #Morpheus00-#Morpheus99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Morpheus%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Morpheus%02d\r", channel_number).c_str());
        }
